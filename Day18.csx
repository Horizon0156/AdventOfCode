...
var pairs = File.ReadAllLines("Data/Day18.txt")
...
var input = File.ReadAllLines("Data/Day18.txt").ToArray();
...
for (var i = 0; i < pairs.Count; i++)
{
    ...
    {
        if (i == j) continue;
        magnitudes.Add(NumberPair.Parse(input[i]).Add(NumberPair.Parse(input[j])).GetMagnitude());
    }
}
